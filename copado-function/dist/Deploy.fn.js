        ...
        var Util2 = class {
            ...
            static provideMCDevTools() {
                ...
                if (!CONFIG2.installMcdevLocally) {
                    ...
                    throw new Error(
                        "Please specify mcdev_version in pipeline & environment settings"
                    );
            ...
            static getBuName(credName, mid) {
                ...
                if (config.credentials[credName] && config.credentials[credName].businessUnits) {
                    const myBuNameArr = Object.keys(
                        config.credentials[credName].businessUnits
                    ).filter((buName) => config.credentials[credName].businessUnits[buName] == mid);
    ...
    "common/Commit.js"(exports, module2) {
        ...
        var Commit2 = class {
            ...
            static commit(originalSelection) {
                const gitDiffArr = execSync2("git diff --staged --name-only")
                    .toString()
                    .split("\n")
                    .map((item) => item.trim())
                    .filter((item) => !!item);
                ...
                if (Array.isArray(gitDiffArr) && gitDiffArr.length) {
                    ...
                    const result = {
                        ...
                        noChangesFound: originalSelection
                            ...
                            .filter((item) => !gitDiffArr.includes(item))
                    };
                    Log2.result(
                        result,
                        `Committed ${
...
var Deploy = class {
    ...
    static getDeployFolder() {
        ...
        const folder =
            (_a = config == null ? void 0 : config.directories) == null ? void 0 : _a.deploy;
        if (!folder) {
