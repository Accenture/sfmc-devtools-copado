...
function getDeployTargetBUs() {
    ...
    if (null == targetBranchBuMapping) {
        throw new Error(
            'Could not find options/deployment/targetBranchBuMapping in ' + configFilePath
        );
    }
...
function deployBU(bu) {
    const ec = execCommandReturnStatus(
        'Deploy BU ' + bu,
...
function promote(toBranch) {
    ...
    //            "Completed cloning branch");
    execCommand(
        'Checking out the branch ' + promotionBranch,
